...
(function () {
  ...
  function encode({ frames, width, height, delayCs = 8, loop = true, onProgress }) {
    ...
    for (let f = 0; f < frames.length; f++) {
      ...
      // Кадр больше не нужен: палитра уже посчитана по гистограмме выше,
      // а пиксели переведены в индексы. Отпускаем его сразу, иначе все
      // кадры (при 480x270 это ~0.5 МБ каждый) держались бы в памяти до
      // конца кодирования и складывались с растущим выходным массивом.
      frames[f] = null;
